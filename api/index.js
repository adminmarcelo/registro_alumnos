...
// Registrar nuevo estudiante Endpoint
app.post('/api/students', (req, res) => {
    const { name, dni, career, tipo, duracion } = req.body;
    if (!/^\d{7,8}$/.test(dni)) {
    return res.status(400).json({ error: "DNI debe contener 7 u 8 dígitos" });
  }
    if (!name || !dni || !career || !tipo || !duracion) {
        return res.status(400).json({ error: "Faltan campos obligatorios: name, dni, career, tipo, duracion." });
    }
...
app.get('/api/students/career', (req, res) => {
    const career = req.query.career;
     console.log("Carrera recibida en el servidor:", career); 
...
app.get('/api/students/:id', (req, res) => {
    ...
    if (!student) {
        return res.status(404).json({ error: "Student not found endpoint." });
    }
...
// Endpoint para buscar estudiantes por nombre (nuevo)
app.get('/api/students', (req, res) => {
    const nameQuery = req.query.name; // Obtiene el parámetro ?name=...
    
    if (nameQuery) {
        // Filtra estudiantes cuyo nombre incluya el texto (case-insensitive)
    ...
    // Si no hay parámetro ?name, devuelve todos los estudiantes (comportamiento existente)
...
app.post('/api/careers', (req, res) => {
    const { name, category, duration, registryNumber } = req.body;

    // Validación básica
    if (!name || !category || !duration || !registryNumber) {
        return res.status(400).json({ error: "Faltan campos obligatorios: name, category, duration o registryNumber." });
    }
    ...
    saveCareers(careers); // Guardar en archivo JSON
...
app.post('/api/contact', (req, res) => {
  ...
  // Aquí deberías implementar el envío real (email, base de datos, etc.)
  return res.status(200).json({ message: "Consulta recibida" });
});